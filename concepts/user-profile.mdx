...
description: "Understand how UK Travel Recommendation builds and updates your preference vector using MHE categories, sentence embeddings, and per-interaction learning rates."
---
...
def _update_profile(self, profile, item):
    ...
    profile.labelEmbed  = ((1 - LABEL_EMBED_ALPHA) * profile.labelEmbed) \
                          + LABEL_EMBED_ALPHA * item.labelEmbed
    profile.summaryEmbed = ((1 - SUMMARY_EMBED_ALPHA) * profile.summaryEmbed) \
                           + SUMMARY_EMBED_ALPHA * item.summaryEmbed
```
