---
title: "Frontend API Client Reference"
...
<Tabs>
  <Tab title="getRecommendations">
    ...
    ```typescript
    export async function getRecommendations(): Promise<Attraction[]> {
  ...
  <Tab title="searchAttractions">
    ...
    ```typescript
    export async function searchAttractions(name: string): Promise<Attraction[]> {
    ...
    | Returns | `Attraction[]` — matching attractions |
  </Tab>
  <Tab title="likeAttraction">
    Records a single right-swipe (like) interaction for a given attraction.

    ```typescript
    export async function likeAttraction(id: string): Promise<void> {
  ...
  <Tab title="dislikeAttraction">
    ...
    ```typescript
    export async function dislikeAttraction(id: string): Promise<void> {
  ...
  <Tab title="addVisitedAttractions">
    ...
    ```typescript
    export async function addVisitedAttractions(ids: string[]): Promise<void> {
  ...
  <Tab title="getLikedAttraction">
    ...
    ```typescript
    export async function getLikedAttraction(): Promise<Attraction[]> {
...
<Tabs>
  ...
  <Tab title="setUserPreferences">
    ...
    The `preferences` array is a binary vector with one element per MHE category (1 = interested, 0 = not interested). The `labels` array contains the human-readable names of the selected categories and is stored for display purposes.
  </Tab>
</Tabs>

## useRecommendations Hook

The `useRecommendations` hook in `hooks/useRecommendation.tsx` is the primary data layer for the `DiscoveryScreen`. It wraps a TanStack Query infinite query and co-locates all swipe interaction logic, so the screen component itself stays thin.
...
const {
  ...
  setCurrentIndex,     // Dispatch<SetStateAction<number>> — update the current index
...
| `setCurrentIndex` | `Dispatch` | Setter for `currentIndex`. Increment this after each swipe to advance the deck. |
| `isLoading` | `boolean` | `true` during the initial fetch. Show a loading indicator while this is `true`. |
| `onSwipeLeft` | `(id: string) => void` | Call with the swiped attraction's ID. Internally calls `dislikeAttraction(id)`. |
| `onSwipeRight` | `(id: string) => void` | Call with the swiped attraction's ID. Internally calls `likeAttraction(id)` and invalidates the `likedHistory` query. |
