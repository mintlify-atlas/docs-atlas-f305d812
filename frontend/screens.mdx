---
title: "App Screens and Navigation Guide"
...
<Steps>
  <Step title="PreferenceCat — Choose Categories">
    Displays the nine **MHE (Mental Health England) attraction categories** as selectable tiles. The user taps the categories they are interested in, and the screen builds two parallel arrays:
  ...
  <Step title="PreferenceArea — Pick a Geographic Area">
    An optional step that lets the user restrict recommendations to a specific region of the UK (e.g. Scotland, South West England). The selection is stored as a query-string fragment in AsyncStorage under the key `geoFilter` (e.g. `?area=Scotland`).

    Users can skip this step; in that case recommendations are drawn from the full UK dataset.
  </Step>
  <Step title="PreferenceImport / AttractionImport — Import Visited Attractions">
    ...
    - This screen is also accessible from Settings (`AttractionImport` route) for users who want to add more visited places later.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Swipe Deck" icon="hand-pointer">
    Powered by `rn-swiper-list`. Each card shows the attraction image, name, category, and a short description. Swipe **right** to like, swipe **left** to dislike. The deck never empties thanks to automatic background pre-fetching.
  </Card>
  ...
  <Card title="Swipe Actions" icon="thumbs-up">
    - **Swipe right** → `onSwipeRight(id)` → `likeAttraction(id)` (POST `/recommendations/like/<id>`) → invalidates the `likedHistory` query so the Liked tab reflects the change immediately.
    - **Swipe left** → `onSwipeLeft(id)` → `dislikeAttraction(id)` (POST `/recommendations/dislike/<id>`).
  </Card>
...
The `SettingsScreen` (accessible from the **Profile** tab) contains two main sub-screens for updating persistent user preferences:
