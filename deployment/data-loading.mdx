---
title: "Loading UK Attraction Data into the Database"
...
<CardGroup cols={2}>
  <Card title="uk_attractions_main_valid_locations.csv" icon="file-csv">
    **Format:** CSV, semicolon-delimited (`;`), UTF-8 encoded
    ...
    | `image_path` | Path or URL to the attraction image |
    | `county` | UK county |
    | `region` | UK region |
...
<Accordion title="Step 1 — Load and join source files">
  ...
  ```python
  csv_df  = pd.read_csv("uk_attractions_main_valid_locations.csv", sep=";", encoding="utf-8")
  pkl_df  = pd.read_pickle("uk_attractions_vectors.pkl")
  ```

  The two DataFrames are joined on the `id` column. Any duplicate columns introduced by the merge (from overlapping column names in both files) are dropped automatically before processing continues.
</Accordion>
...
<Accordion title="Step 4 — Bulk insert in batches of 1 000">
  ...
  ```python
  Attraction.objects.bulk_create(batch, batch_size=1000)
  ```
</Accordion>
